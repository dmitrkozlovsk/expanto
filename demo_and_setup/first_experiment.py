...
def main():
    """
    Main function to set up the database and seed it with initial data.
    """
    parser = argparse.ArgumentParser(description="Setup Expanto database with demo data.")
    parser.add_argument("--force", action="store_true", help="Force recreate the database if it already exists.")
    args = parser.parse_args()

    db_path = "expanto.db"

    if os.path.exists(db_path) and not args.force:
        print(f"Database '{db_path}' already exists. Skipping database creation and data seeding.")
        print("To overwrite, run: make setup force=true")
        return
    ...
    # Create all tables
    print("Setting up new database and seeding demo data...")
    ...
    print(
        f"Created: {total_experiments} experiments, {total_observations} observations, {total_jobs} jobs, {total_precomputes} precomputed metrics"
    )
    session.close()
